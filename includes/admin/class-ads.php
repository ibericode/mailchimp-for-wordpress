...
class PL4WP_Admin_Ads {
	...
	public function add_hooks() {
		...
		if( defined( 'PL4WP_PREMIUM_VERSION' ) ) {
			return false;
		}

		add_filter( 'pl4wp_admin_plugin_meta_links', array( $this, 'plugin_meta_links' ) );
		add_action( 'pl4wp_admin_form_after_behaviour_settings_rows', array( $this, 'after_form_settings_rows' ) );
		add_action( 'pl4wp_admin_form_after_appearance_settings_rows', array( $this, 'after_form_appearance_settings_rows' ) );
		add_action( 'pl4wp_admin_sidebar', array( $this, 'admin_sidebar' ) );
		add_action( 'pl4wp_admin_footer', array( $this, 'admin_footer' ) );
		add_action( 'pl4wp_admin_other_settings', array( $this, 'ecommerce' ), 90 );

		add_filter( 'pl4wp_admin_menu_items', array( $this, 'add_menu_item' ) );

		add_action( 'pl4wp_admin_after_woocommerce_integration_settings', array( $this, 'ecommerce' ) );
		return true;
	}

	public function add_menu_item( $items ) {
		$items['extensions'] = array(
			'title' => __( 'Add-ons', 'phplist-for-wp' ),
			'text' => '<span style="color: #c44;">' . __( 'Add-ons', 'phplist-for-wp' ) . '</span>',
			'slug' => 'extensions',
	...
	public function admin_sidebar() {
		...
		?>
		<div class="pl4wp-box" id="pl4wp-optin-box">

			<?php $user = wp_get_current_user(); ?>
			<!-- Begin PhpList Signup Form -->
			<div id="mc_embed_signup">
				...
				<form action="//pl4wp.us1.list-manage.com/subscribe/post?u=a2d08947dcd3683512ce174c5&amp;id=a940232df9" method="post" id="mc-embedded-subscribe-form" name="mc-embedded-subscribe-form" target="_blank">
					...
					<p>
						<label for="pl4wp-fname"><?php _e( 'First Name', 'phplist-for-wp' ); ?></label>
						<input type="text" value="<?php echo esc_attr( $user->user_firstname ); ?>" name="FNAME" class="regular-text" id="pl4wp-fname">
					</p>
					<div style="position: absolute; left: -5000px;">
						<input type="text" name="b_a2d08947dcd3683512ce174c5_a940232df9" tabindex="-1" value="" autocomplete="off" />
					</div>
					<p>
						<input type="submit" value="<?php esc_attr_e( 'Subscribe', 'phplist-for-wp' ); ?>" name="subscribe" class="button">
					</p>
	...
	public function show_extensions_page() {
		require PL4WP_PLUGIN_DIR . 'includes/views/extensions.php';
	}
