...
class PL4WP_PhpList {
	...
	*/
	public function list_subscribe( $list_id, $email_address, array $args = array(), $update_existing = false, $replace_interests = true ) {
		...
		} catch( PL4WP_API_Resource_Not_Found_Exception $e ) {
	...
	*/
	public function empty_cache() {
		...
		delete_option( 'pl4wp_phplist_list_ids' );
		$wpdb->query( "DELETE FROM {$wpdb->options} WHERE option_name LIKE 'pl4wp_phplist_list_%'" );
		delete_transient( 'pl4wp_list_counts' );
	}

	/**
	* Get PhpList lists from cache.
	*
	...
	*/
	...
	/**
	* Get a specific PhpList list from local DB.
	*
	* @param string $list_id
	* @return PL4WP_PhpList_List
	*/
	...
	/**
	* Get PhpList lists, from cache or remote API.
	*
	* @param boolean $force Whether to force a result by hitting PhpList API
	* @return array
	*/
	...
	*/
	private function fetch_list( $list_id ) {
		try{
			...
			if( $list_data->stats->merge_field_count > 0 ) {
				...
				foreach( $field_data as $data ) {
					$object = PL4WP_PhpList_Merge_Field::from_data( $data );
			...
			foreach( $interest_categories_data as $interest_category_data ) {
				$interest_category = PL4WP_PhpList_Interest_Category::from_data( $interest_category_data );
		...
		// save in option
		update_option( 'pl4wp_phplist_list_' . $list_id, $list, false );
		return $list;
	}

	/**
	* Get PhpList list ID's
	*
	* @param bool $force Force result by hitting PhpList API
	* @return array
	*/
	public function get_list_ids( $force = false ) {
		$list_ids = (array) get_option( 'pl4wp_phplist_list_ids', array() );
	...
	*/
	public function fetch_list_ids() {
		...
		// store list id's
		update_option( 'pl4wp_phplist_list_ids', $list_ids, false );

		return $list_ids;
	}

	/**
	* Fetch list ID's + lists from PhpList.
	*
	* @return bool
	*/
	...
	*/
	public function get_list( $list_id, $force = false ) {
		$list = get_option( 'pl4wp_phplist_list_' . $list_id );
		...
		if( empty( $list ) ) {
			return new PL4WP_PhpList_List( $list_id, 'Unknown List' );
		}
	...
	*/
	public function get_subscriber_counts() {
		...
		// get from transient
		$list_counts = get_transient( 'pl4wp_list_counts' );
		...
		/**
		* Filters the cache time for PhpList lists configuration, in seconds. Defaults to 3600 seconds (1 hour).
		*
		* @since 2.0
		* @param int $seconds
		*/
		$transient_lifetime = (int) apply_filters( 'pl4wp_lists_count_cache_time', $seconds );
		set_transient( 'pl4wp_list_counts', $list_counts, $transient_lifetime );
	...
	*/
	public function get_subscriber_count( $list_ids ) {
		...
		*/
		return apply_filters( 'pl4wp_subscriber_count', $count, $list_ids );
	}
