...
class PL4WP_API_v3_Client {
  ...
  */
  private function request( $method, $resource, array $data = array() ) {
    ...
    if( empty( $this->api_key ) ) {
      throw new PL4WP_API_Exception( "Missing API key", 001 );
    }
    ...
    * @param array $args
    */
    $args = apply_filters( 'pl4wp_http_request_args', $args, $url );
  ...
  */
  private function get_headers() {
    ...
    $headers = array();
    $headers['Authorization'] = 'Basic ' . base64_encode( 'pl4wp:' . $this->api_key );
    $headers['Accept'] = 'application/json';
    $headers['Content-Type'] = 'application/json';
    $headers['User-Agent'] = 'pl4wp/' . PL4WP_VERSION . '; WordPress/' . $wp_version . '; ' . get_bloginfo( 'url' );
  ...
  */
  private function parse_response( $response ) {
    ...
    if( $response instanceof WP_Error ) {
      throw new PL4WP_API_Connection_Exception( $response->get_error_message(), (int) $response->get_error_code() );
    }
    ...
    // set body to "true" in case PhpList returned No Content
    ...
    if( $code >= 400 ) {
      ...
      if( is_object( $data ) && isset( $data->type ) && $data->type === 'akamai_error_message' ) {
        throw new PL4WP_API_Connection_Exception( $message, $code, $this->last_request, $this->last_response, $data );
      }

      if( $code === 404 ) {
        throw new PL4WP_API_Resource_Not_Found_Exception( $message, $code, $this->last_request, $this->last_response, $data );
      }

      // phplist returned an error..
      throw new PL4WP_API_Exception( $message, $code, $this->last_request, $this->last_response, $data );
    }
    ...
    throw new PL4WP_API_Exception( $message, $code, $this->last_request, $this->last_response );
  }
