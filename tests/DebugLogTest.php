...
class DebugLogTest extends TestCase {
	 ...
	 * @var string
	 */
	private $file = '/tmp/pl4wp-debug.log';

	/**
	 * @covers PL4WP_Debug_Log::log
	 */
	public function test_log() {
		$logger = new PL4WP_Debug_Log( $this->file, 0 );
	...
	/**
	 * @covers PL4WP_Debug_Log::debug
	 */
	public function test_debug() {
		$message = "Sample debug message";

		// level is 200, debug is 100 so this shouldn't log
		$logger = new PL4WP_Debug_Log( $this->file, 200 );
		self::assertFalse( $logger->debug( $message ) );

		// level is 100, debug is 100 so this should log
		$logger = new PL4WP_Debug_Log( $this->file, 100 );
		self::assertTrue( $logger->debug( $message ) );

		// test string
		$logger = new PL4WP_Debug_Log( $this->file, 'debug' );
		self::assertTrue( $logger->debug( $message ) );

		// test string
		$logger = new PL4WP_Debug_Log( $this->file, 'warning' );
		self::assertFalse( $logger->debug( $message ) );
	}

	/**
	 * @covers PL4WP_Debug_Log::info
	 */
	public function test_info() {
		$message = "Sample info message";

		// this shouldn't log (info level is 200)
		$logger = new PL4WP_Debug_Log( $this->file, 300 );
		...
		// test string
		$logger = new PL4WP_Debug_Log( $this->file, 'info' );
		self::assertTrue( $logger->info( $message ) );
	...
	/**
	 * @covers PL4WP_Debug_Log::warning
	 */
	public function test_warning() {
		$message = "Sample warning message";

		// this shouldn't log (warning level is 300)
		$logger = new PL4WP_Debug_Log( $this->file, 400 );
		...
		// test string with higher level
		$logger = new PL4WP_Debug_Log( $this->file, 'error' );
		self::assertFalse( $logger->warning( $message ) );
	}


	/**
	 * @covers PL4WP_Debug_Log::error
	 */
